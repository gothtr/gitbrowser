...
- [ ] Анимация пульсации прогресс-бара в downloads.html
- [ ] Отображение скорости и ETA в UI загрузок
- [ ] Кнопки "Открыть файл" / "Показать в папке" после завершения
- [ ] Уведомление (toast) при завершении загрузки
...
### Исправления
- [ ] Передавать `visit_time` как ISO строку или миллисекунды из RPC
- [ ] Записывать историю после `page-title-updated` (когда title уже известен), а не в `did-navigate`
- [ ] Добавить группировку по дням в history.html
- [ ] Добавить favicon в записи истории
...
### Исправления
- [ ] Увеличить кнопки тулбара до 34px, добавить плавные hover-эффекты
- [ ] Переработать newtab: приветствие по времени суток, часы, погода-виджет (опционально), красивая типографика
- [ ] Добавить анимации появления элементов на всех внутренних страницах (stagger fade-in)
- [ ] Улучшить карточки quick-links: показывать favicon сайта вместо буквы
- [ ] Добавить возможность менять фон newtab (solid colors, без градиентов)
- [ ] Сделать более крупные и читаемые элементы на страницах bookmarks/history/downloads
- [ ] Чистый, минималистичный стиль — без фиолетовых/градиентных акцентов
...
### Исправления
- [ ] Загружать локаль при старте приложения (до создания окна)
- [ ] Локализовать все hardcoded строки в HTML-страницах
- [ ] Локализовать `getInternalTitle()` через кэшированную локаль
- [ ] Добавить недостающие ключи в en.json и ru.json (context_menu.save_image, context_menu.copy_image_url, context_menu.open_image_new_tab, passwords.*, extensions.*)
- [ ] Локализовать toast-сообщения
...
### Исправления
- [ ] Добавить очистку downloads Map (удалять записи старше 24ч)
- [ ] Добавить reconnect в RustBridge при падении процесса
- [ ] Добавить graceful error handling при потере связи с backend
- [ ] Добавить health check (ping) для RPC connection
- [ ] Очищать event listeners при закрытии вкладок

---

## 10. Мелкие баги и недоработки

- [ ] `getInternalTitle()` возвращает hardcoded английские названия — нужна локализация
...
- [ ] Bookmark добавляется с пустым title (передаётся `title: ''` из тулбара)
- [ ] Zoom indicator не показывает текущий уровень при переключении вкладок
- [ ] Find bar не закрывается при переключении вкладок
- [ ] Нет Ctrl+Tab для переключения между вкладками
- [ ] Нет middle-click для открытия ссылки в новой вкладке
- [ ] Toast сообщения не локализованы
- [ ] Нет подтверждения при очистке истории
